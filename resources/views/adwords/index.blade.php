            ...
            $('.datatables').DataTable(
                {
                    ...
                    'filter' : "Manager_Id ne null" + customFilters +
                        " and (ContractType/Name eq 'Adwords' or ContractType/Name eq 'Adwords Local' or ContractType/Id eq 18 or ContractType/Id eq 20 or ContractType/Id eq 15)", // 18 is Support
                    "sAjaxSource": api_address+"Contracts?$expand=ClientAlias($select=Id,Name),Country,Product($select=Id,Name,Description),User($select=FullName),Manager($select=FullName),InformationSchemes",
                    "aoColumns": [
                        {mData:"Id","sType":"numeric","width":"7%",mRender:function(id){
                            return '<a href="'+base_url+'/adwords/show/'+ id+'" title="'+Lang.get('labels.see-contract')+'">'+id+'</a>';
                            }
                        },
                        {"sType":"string","mData":null,"oData":"ClientAlias/Name",mRender:function(data){
                            return '<a href="'+base_url+'/adwords/show/'+ data.Id +'" title="'+Lang.get('labels.see-client')+'">'+data.ClientAlias.Name+'</a>';
                            }
                        },
                        ...
                        {"mData":null,"sType":"string","oData":"Product/Name",mRender:function(data){
                            if(data.Product != null) {return '<a href="'+base_url+'/adwords/show/'+ data.Id +'" title="'+Lang.get('labels.see-client')+'">'+data.Product.Name+'</a>'; }else {return "----"}
                            }
                        },
        ...
        <div class="col-md-12">
            <div class="panel panel-adwords">
                <div class="panel-heading">
                    <h4><i class="fa fa-file"></i> AdWords @lang('labels.contracts')</h4>
                ...
                <div class="panel-body">
                        ...
                        <div class="col-md-6 col-md-offset-6">
                            <div class="">
                                <div class="form-group-sm col-md-3">
                                    {!! Form::select('User_Id',withEmpty($sellers,Lang::get('labels.select-seller')),'User_Id eq \''.Auth::user()->externalId."'",['class'=>'form-control adwordsFilters']) !!}
                                </div>
                                <div class="form-group-sm col-md-3">
                                    {!! Form::select('Manager_Id',withEmpty($managers,Lang::get('labels.select-assigned')),"Manager_Id eq '".Auth::user()->externalId."'",['class'=>'form-control adwordsFilters']) !!}
                                </div>
                                <div class="form-group-sm col-md-3">
                                    {!! Form::select('TeamStatus',withEmpty($teamStatus,Lang::get('labels.team-status')),null,['class'=>'form-control adwordsFilters']) !!}
                                </div>
                                <div class="form-group-sm col-md-3">
                                    {!! Form::select('Status',withEmpty($contractStatus,Lang::get('labels.contract-status')),"Status eq 'Active'",['class'=>'form-control adwordsFilters']) !!}
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <table width="99%" cellpadding="0" cellspacing="0" border="0" id="table-list" class="table datatables">
                        <thead>
                            <tr>
